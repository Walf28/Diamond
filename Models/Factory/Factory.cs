...
namespace MyCompany
{
    public class Factory : FactoryObject
    {
        ...
        /*public override bool Save()
        {
            ...
            if (id == null)
            {
                if (DB.Insert("Factory", [name, AcceptedRequests, ""], out int? returnID))
                {
                    id = returnID;
                    return true;
                }
            ...
            return DB.Replace("Factory", "id", id.ToString()!,
        ...
        public override bool Delete()
        {
            ...
            return DB.Delete("Factory", "id", id.ToString()!);
        }
        ...
        {
            // Исходные данные
            List<RegionType> TechnologyProcess = request.GetNeedTechnologyProcessing;
        ...
        public List<Route> GetRoutes(string Product, int size = 0)
        {
            ...
            List<Region> regions = LoadRegions();
            List<RegionType> ListTechnology = [];
            foreach (var st in StringTechnology)
                ListTechnology.Add((RegionType)int.Parse(st));
        ...
        private static List<Route> FindRoute(Region region, List<RegionType> technology, string nowPath = "") // Проверяем соответствие участка на технологию для построения маршрута
        {
            List<Route> routes = [];
